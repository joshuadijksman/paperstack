...
with open(f'/Users/noor/Documents/GitHub/2025-Projects_Team-1-Bouncing-on-Paper-The-Surprising-Restitution-Coefficient-of-a-Paper-Stack-/Code/1-1-M.csv', 'r') as yurr:
    ...
    # fout op tijd 
    fout_frame_lijst = []
    for i in range(0, len(frame_lijst)):
        fout_frame_lijst.append(1)
    
    print(fout_frame_lijst)
    ...
    plt.plot(frame_lijst, hoogte_lijst, 'r-')
    plt.xlabel('aantal frames')
    plt.ylabel('hoogte in pixels')
    plt.xlim(0.1,0.3)
    plt.errorbar(frame_lijst, hoogte_lijst, xerr = fout_frame_lijst, yerr = fout_hoogte_lijst)
    plt.show()
    ...
    for element in range(0, len(snelheden_lijst)):
        fout_snelheid = snelheden_lijst[element] * ((fout_hoogte_lijst[element]/hoogte_lijst[element])**2 + (fout_frame_lijst[element]/frame_lijst[element])**2)**0.5
    ...
    for i in range(1, len(snelheden_lijst)):
        if snelheden_lijst[i - 1] > 0 and snelheden_lijst[i] < 0 and len(maxima_tijden) <= 3 and i > 50:  # filter op realistische waarde
            maxima_tijden.append(frame_lijst[i])
            ...
            schatting_max_tijd = frame_lijst[i - 1] + (snelheden_lijst[i - 1]/(snelheden_lijst[i - 1] - snelheden_lijst[i])) * 0.005
...
# gemiddelde van de 3 data punten = 
