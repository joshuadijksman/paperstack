...
def parabola_fit(frames, y_points, Plot, fit_report):
    ...
    if Plot:
        ...
        ax_main.plot([min(frames), max(frames)], [b, b],
...
# Making a better working COR calculator that works by fitting two seperate parabolas to find the ACTUAL pixel at which the ball hits the paper.

def COR_calculator_2(inputfolder, variable_type, variable_value, filename, Find_Plot, Fit_Plot, Fit_Report):
    # tweak these
    leniency = 5
    y_err = 1
...
######################## Outdated functions replaced by better versions #############################
...
def COR_calculator_general(inputfolder, variable_type, variable_value, filename, Find_Plot, Fit_Plot, Fit_Report):
    ...
    a, bounce_height, t_0 = parabola_fit(frame_bounce, y_bounce, Fit_Plot, Fit_Report)
    drop_height = drop_height
...
############################## Specific functions for specific files ###############################
...
def calulate_COR(networkfolder, filebegin, thicknesslist, repetitions, Plot):
    ...
    for thickness in thicknesslist:
        ...
        for i in range(repetitions):
            ...
            _, bounce_height, _ = parabola_fit(frames, y_points, False, False)
